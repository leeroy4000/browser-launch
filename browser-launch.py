...
def setup_logging(log_file=DEFAULT_LOG_FILE):
    ...
    Can be called multiple times - updates the file handler if the path changes.
    ...
    # Remove any existing file handlers to avoid duplicates on re-call
...
def add_captive_portal():
    ...
    # Convert legacy single-portal format if present
...
def run_setup_wizard():
    ...
    if add_tabs:
        ...
        while True:
            window_name = input("Window name (e.g., 'Network', 'Media') [leave blank to finish]: ").strip()
            if not window_name:
                break
            ...
            while True:
                url = input("  URL [leave blank to finish window]: ").strip()
                if not url:
                    break
    ...
    if not install_service:
        print("To run manually:        ./browser-launch.py --run")
        print("To install service:     ./browser-launch.py --install")
...
Examples:
  ...
  ./browser-launch.py --add-portal Add new captive portal to config
    ...
    # Re-initialize logging with path from config if it differs from default
    settings = config.get('settings', {})
    config_log_file = settings.get('log_file', DEFAULT_LOG_FILE)
    if config_log_file != DEFAULT_LOG_FILE:
