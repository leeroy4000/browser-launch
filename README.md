...
- [ ] **This repo (`leeroy4000/browser-launch`) is PUBLIC** (every other repo is private, checked 2026-09-24). `CLAUDE.md` was added and then deleted on GitHub 3 minutes later on 2026-09-05 (via GitHub's web/API path, under your account; nobody can say who did it, and Nathan doesn't remember it). Keeping it out of the repo is sensible for a public repo, so the local `CLAUDE.md` stays **untracked** and is listed in `.git/info/exclude`. Only change that if you decide you want it public. Never put IPs, hostnames or other homelab internals in this repo's README either.
